...
{
    public AddUserToGroupValidator(WikiContext context, IAuthorizedUserProvider authorizedUserProvider)
    {
        RuleFor(r => r.GroupId)
            .Cascade(CascadeMode.Stop)
            .EntityShouldExist(context.Groups)
            .WithErrorCode(FailCode.BadRequest.ToString())
            .UserShouldHaveAccessToGroup(context.Groups, authorizedUserProvider, GroupAccessRule.AddMembers)
        ...
        RuleFor(r => r)
            .CustomAsync(async (request, ctx, _) =>
            {
                ...
                if (group.Users.Any())
                {
                    ctx.AddFailure("User already in group");
                }
            });
    }
}